            ...
            <div className={`auth-container ${isSignUpActive ? 'right-panel-active' : ''} ${resolvedTheme === 'obsidian' || resolvedTheme === 'hydra' ? '!bg-white/5 !backdrop-blur-xl border border-white/20 shadow-2xl' : ''}`} style={{ minHeight: '600px' }}>
                ...
                >
                    <form onSubmit={handleRegisterSubmit} className="auth-form" style={{ height: '100%', justifyContent: 'center' }}>
                        ...
                        <span className="text-gray-500 dark:text-gray-400 mb-4">or use your email for registration</span>
                ...
                >
                    <form onSubmit={handleLoginSubmit} className="auth-form" style={{ height: '100%', justifyContent: 'center' }}>
                        ...
                        <span className="text-gray-500 dark:text-gray-400 mb-4">or use your account</span>
                        ...
                        <div className={`auth-input-group ${loginPassword ? 'is-filled' : ''}`}>
                             <input ref={signInPasswordRef} className="auth-input bg-surface-soft text-text-primary border-border" type="password" value={loginPassword} onChange={e => setLoginPassword(e.target.value)} required placeholder=" " />
                            <span className="bar"></span>
                            <label className="auth-label">Password</label>
                        </div>
                        {error && !isSignUpActive && <p className="text-red-500 text-xs mt-2">{error}</p>}
                        <Link to="/forgot-password" className="text-xs my-4 text-gray-500 dark:text-gray-400 hover:underline">Forgot your password?</Link>
