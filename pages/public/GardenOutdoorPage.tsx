...
const GardenOutdoorPage: React.FC = () => {
    ...
    useEffect(() => {
        ...
        itemService.getItems({ category: 'garden-outdoor' }, { page: 1, limit: 8 })
            .then(data => {
                // Mock filtering for demonstration
                setFurniture(data.items.slice(0, 4));
    ...
    return (
        <>
            ...
            <div className="bg-white dark:bg-dark-background animate-fade-in-up">
                <section ref={heroRef} className="animate-reveal relative h-[50vh] bg-green-600">
                     <img src="https://picsum.photos/seed/garden-hero/1920/1080" alt="Lush garden" className="absolute inset-0 w-full h-full object-cover opacity-50"/>
                    <div className="relative h-full flex flex-col items-center justify-center text-center p-4">
                        <h1 className="text-5xl md:text-6xl font-extrabold font-serif-display text-white">Create Your Outdoor Oasis</h1>
                        <p className="mt-4 text-lg max-w-xl text-green-100">Everything you need to cultivate a beautiful garden and a relaxing outdoor living space.</p>
                    </div>
                </section>

                <main className="container mx-auto px-4 sm:px-6 lg:px-8 py-16 space-y-16">
                    <section>
                        <SectionHeader title="Patio Furniture" linkTo="/browse?category=garden-outdoor" />
                        <ProductGrid items={furniture} onQuickView={setQuickViewItem} isLoading={isLoading} />
                    </section>
                    
                    <section>
                        <SectionHeader title="Gardening Tools" linkTo="/browse?category=garden-outdoor" />
