...
const CurtainsBlindsPage: React.FC = () => {
    const heroRef = useScrollReveal<HTMLDivElement>();
    
    const [curtains, setCurtains] = useState<Item[]>([]);
    const [blinds, setBlinds] = useState<Item[]>([]);
    const [isLoading, setIsLoading] = useState(true);
    const [quickViewItem, setQuickViewItem] = useState<Item | null>(null);

    useEffect(() => {
        // FIX: Added missing page property to the pagination object.
        itemService.getItems({ category: 'curtains-blinds' }, { page: 1, limit: 8 })
            .then(data => {
                // Mock filtering for demonstration
                setCurtains(data.items.slice(0, 4));
                setBlinds(data.items.slice(4, 8));
                setIsLoading(false);
            });
    }, []);
        ...
        <>
            ...
            <div className="bg-white dark:bg-dark-background animate-fade-in-up">
                <section ref={heroRef} className="animate-reveal relative h-[50vh] bg-gray-50">
                     <img src="https://picsum.photos/seed/curtains-hero/1920/1080" alt="Sunlit window with curtains" className="absolute inset-0 w-full h-full object-cover"/>
                    <div className="relative h-full flex flex-col items-center justify-center text-center p-4 bg-white/40 dark:bg-black/40">
                        <h1 className="text-5xl md:text-6xl font-extrabold font-serif-display text-gray-800 dark:text-white">Frame Your View</h1>
                        <p className="mt-4 text-lg max-w-xl text-gray-700 dark:text-gray-200">Control light and add privacy with our stylish collection of curtains and blinds.</p>
                    </div>
                </section>

                <main className="container mx-auto px-4 sm:px-6 lg:px-8 py-16 space-y-16">
                    <section>
                        <SectionHeader title="Curtains" linkTo="/browse?category=curtains-blinds" />
                        <ProductGrid items={curtains} onQuickView={setQuickViewItem} isLoading={isLoading} />
                    </section>
                    
                    <section>
                        <SectionHeader title="Blinds & Shades" linkTo="/browse?category=curtains-blinds" />
                        <ProductGrid items={blinds} onQuickView={setQuickViewItem} isLoading={isLoading} />
                    </section>
                </main>
            </div>
        </>
...
export default CurtainsBlindsPage;