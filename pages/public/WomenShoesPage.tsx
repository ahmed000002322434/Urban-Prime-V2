...
const WomenShoesPage: React.FC = () => {
    const heroRef = useScrollReveal<HTMLDivElement>();
    
    const [heels, setHeels] = useState<Item[]>([]);
    const [flats, setFlats] = useState<Item[]>([]);
    const [isLoading, setIsLoading] = useState(true);
    const [quickViewItem, setQuickViewItem] = useState<Item | null>(null);

    useEffect(() => {
        // FIX: Property 'page' is missing in type '{ limit: number; }'.
        itemService.getItems({ category: 'womens-shoes' }, { page: 1, limit: 8 })
            .then(data => {
                // Mock filtering
                setHeels(data.items.slice(0, 4));
                setFlats(data.items.slice(4, 8));
                setIsLoading(false);
            });
    }, []);
        ...
        <>
            ...
            <div className="bg-purple-50/20 dark:bg-dark-background animate-fade-in-up">
                ...
                <section ref={heroRef} className="animate-reveal container mx-auto px-4 sm:px-6 lg:px-8 py-20 md:py-24 text-center">
                    <h1 className="text-5xl md:text-7xl font-serif-display text-gray-900 dark:text-dark-text">Women's Shoe Collection</h1>
                    <p className="mt-6 text-lg text-gray-600 dark:text-gray-400 max-w-2xl mx-auto">Where elegance meets everyday comfort.</p>
                </section>
                
                <main className="container mx-auto px-4 sm:px-6 lg:px-8 py-16 space-y-16">
                    <section>
                        <SectionHeader title="Heels & Pumps" linkTo="/browse?category=womens-shoes" />
                        <ProductGrid items={heels} onQuickView={setQuickViewItem} isLoading={isLoading} />
                    </section>
                    
                    <section>
                        <SectionHeader title="Flats & Sneakers" linkTo="/browse?category=womens-shoes" />
                        <ProductGrid items={flats} onQuickView={setQuickViewItem} isLoading={isLoading} />
                    </section>
                </main>
            </div>
        </>
