...
const JewelryPage: React.FC = () => {
  ...
  const jewelryCategory = allCategories.find(c => c.id === 'jewelry');
  const subCategories = jewelryCategory?.subcategories || [];
  
  return (
    <div className="bg-blue-50/20 dark:bg-dark-background animate-fade-in-up">
      ...
      <section ref={heroRef} className="animate-reveal container mx-auto px-4 sm:px-6 lg:px-8 py-20 md:py-32 text-center">
        <h1 className="text-5xl md:text-7xl font-serif-display text-gray-900 dark:text-dark-text">Exquisite Jewelry</h1>
        <p className="mt-6 text-lg text-gray-600 dark:text-gray-400 max-w-2xl mx-auto">Find the perfect piece to celebrate life's moments, from timeless classics to modern designs.</p>
      </section>

      <section ref={categoriesRef} className="animate-reveal container mx-auto px-4 sm:px-6 lg:px-8 pb-24">
        <h2 className="text-4xl font-serif-display text-center mb-12 dark:text-dark-text">Shop by Piece</h2>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
          {subCategories.map((cat) => (
            <SubCategoryCard 
                key={cat.id} 
                title={cat.name} 
                link={cat.id === 'rings' ? '/rings' : `/browse?category=${cat.id}`}
