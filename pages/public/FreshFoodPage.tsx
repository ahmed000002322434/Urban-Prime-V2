...
const FreshFoodPage: React.FC = () => {
    ...
    useEffect(() => {
        ...
        itemService.getItems({ category: 'fresh-food' }, { page: 1, limit: 8 })
            .then(data => {
                // Mock filtering for demonstration
                setFruits(data.items.slice(0, 4));
                setVegetables(data.items.slice(4, 8));
                setIsLoading(false);
            });
    }, []);
    ...
    return (
        <>
            ...
            <div className="bg-white dark:bg-dark-background animate-fade-in-up">
                <section ref={heroRef} className="animate-reveal relative h-[50vh] bg-green-100">
                     <img src="https://picsum.photos/seed/fresh-hero/1920/1080" alt="Fresh fruits and vegetables" className="absolute inset-0 w-full h-full object-cover"/>
                    <div className="relative h-full flex flex-col items-center justify-center text-center p-4 bg-black/20">
                        <h1 className="text-5xl md:text-6xl font-extrabold font-serif-display text-white">Farm to Table</h1>
                        <p className="mt-4 text-lg max-w-xl text-gray-100">The best of the season, delivered fresh. Explore our selection of fruits and vegetables.</p>
                    </div>
                </section>

                <main className="container mx-auto px-4 sm:px-6 lg:px-8 py-16 space-y-16">
                    <section>
                        <SectionHeader title="Fresh Fruits" linkTo="/browse?category=fresh-food" />
                        <ProductGrid items={fruits} onQuickView={setQuickViewItem} isLoading={isLoading} />
                    </section>
                    
                    <section>
                        <SectionHeader title="Fresh Vegetables" linkTo="/browse?category=fresh-food" />
                        <ProductGrid items={vegetables} onQuickView={setQuickViewItem} isLoading={isLoading} />
                    </section>
                </main>
            </div>
        </>
