...
const ElectronicsPage: React.FC = () => {
  ...
  const electronicsCategory = allCategories.find(c => c.id === 'electronics');
  const subCategories = electronicsCategory?.subcategories || [];
  
  return (
    <div className="bg-gray-50/50 dark:bg-dark-background animate-fade-in-up">
      <section ref={heroRef} className="animate-reveal container mx-auto px-4 sm:px-6 lg:px-8 py-20 md:py-32 text-center">
        <h1 className="text-5xl md:text-7xl font-extrabold font-display text-gray-900 dark:text-dark-text">Latest in Tech</h1>
        <p className="mt-6 text-lg text-gray-600 dark:text-gray-400 max-w-2xl mx-auto">From professional gear to everyday gadgets, rent or buy the electronics you need.</p>
      </section>
