...
const BathEssentialsPage: React.FC = () => {
    const heroRef = useScrollReveal<HTMLDivElement>();
    
    const [towels, setTowels] = useState<Item[]>([]);
    ...
    useEffect(() => {
        ...
        itemService.getItems({ category: 'bath-essentials' }, { page: 1, limit: 8 })
            .then(data => {
                // Mock filtering for demonstration
                setTowels(data.items.slice(0, 4));
                setAccessories(data.items.slice(4, 8));
                setIsLoading(false);
            });
    }, []);
    ...
    return (
        <>
            ...
            <div className="bg-white dark:bg-dark-background animate-fade-in-up">
                <section ref={heroRef} className="animate-reveal relative h-[50vh] bg-cyan-50">
                     <img src="https://picsum.photos/seed/bath-hero/1920/1080" alt="Modern bathroom" className="absolute inset-0 w-full h-full object-cover"/>
                    <div className="relative h-full flex flex-col items-center justify-center text-center p-4 bg-black/20">
                        <h1 className="text-5xl md:text-6xl font-extrabold font-serif-display text-white">Create Your At-Home Spa</h1>
                        <p className="mt-4 text-lg max-w-xl text-gray-100">Elevate your daily routine with plush towels, stylish accessories, and bath essentials.</p>
                    </div>
                </section>

                <main className="container mx-auto px-4 sm:px-6 lg:px-8 py-16 space-y-16">
                    <section>
                        <SectionHeader title="Towels & Bath Linens" linkTo="/browse?category=bath-essentials" />
                        <ProductGrid items={towels} onQuickView={setQuickViewItem} isLoading={isLoading} />
                    </section>
                    
                    <section>
                        <SectionHeader title="Bath Accessories" linkTo="/browse?category=bath-essentials" />
...
export default BathEssentialsPage;