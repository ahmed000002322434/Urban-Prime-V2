...
const CarpetsRugsPage: React.FC = () => {
    const heroRef = useScrollReveal<HTMLDivElement>();
    
    const [modern, setModern] = useState<Item[]>([]);
    const [traditional, setTraditional] = useState<Item[]>([]);
    const [isLoading, setIsLoading] = useState(true);
    const [quickViewItem, setQuickViewItem] = useState<Item | null>(null);

    useEffect(() => {
        // FIX: Added missing page property to the pagination object.
        itemService.getItems({ category: 'carpets-rugs' }, { page: 1, limit: 8 })
            .then(data => {
                // Mock filtering for demonstration
                setModern(data.items.slice(0, 4));
                setTraditional(data.items.slice(4, 8));
                setIsLoading(false);
            });
    }, []);
        ...
        <>
            ...
            <div className="bg-white dark:bg-dark-background animate-fade-in-up">
                <section ref={heroRef} className="animate-reveal relative h-[50vh] bg-gray-200">
                     <img src="https://picsum.photos/seed/rugs-hero/1920/1080" alt="Room with a beautiful rug" className="absolute inset-0 w-full h-full object-cover"/>
                    <div className="relative h-full flex flex-col items-center justify-center text-center p-4 bg-black/20">
                        <h1 className="text-5xl md:text-6xl font-extrabold font-serif-display text-white">Ground Your Space in Style</h1>
                        <p className="mt-4 text-lg max-w-xl text-gray-100">Discover the perfect carpet or rug to add warmth, texture, and personality to any room.</p>
                    </div>
                </section>

                <main className="container mx-auto px-4 sm:px-6 lg:px-8 py-16 space-y-16">
                    <section>
                        <SectionHeader title="Modern & Contemporary" linkTo="/browse?category=carpets-rugs" />
                        <ProductGrid items={modern} onQuickView={setQuickViewItem} isLoading={isLoading} />
                    </section>
                    
                    <section>
                        <SectionHeader title="Traditional & Vintage" linkTo="/browse?category=carpets-rugs" />
                        <ProductGrid items={traditional} onQuickView={setQuickViewItem} isLoading={isLoading} />
                    </section>
                </main>
            </div>
        </>
...
export default CarpetsRugsPage;