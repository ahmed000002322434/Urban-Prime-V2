-- Repairs the commerce/dropshipping schema expected by /commerce/seller/bookings.
-- Run this in Supabase SQL editor if seller bookings fail with:
--   column supplier_orders.order_item_id does not exist
...
do $$ begin alter type public.supplier_order_status add value if not exists 'pending_review'; exception when duplicate_object then null; end $$;
do $$ begin alter type public.supplier_order_status add value if not exists 'approved'; exception when duplicate_object then null; end $$;
do $$ begin alter type public.supplier_order_status add value if not exists 'submitted'; exception when duplicate_object then null; end $$;
do $$ begin alter type public.supplier_order_status add value if not exists 'accepted'; exception when duplicate_object then null; end $$;
do $$ begin alter type public.supplier_order_status add value if not exists 'failed'; exception when duplicate_object then null; end $$;
do $$ begin alter type public.supplier_order_status add value if not exists 'returned'; exception when duplicate_object then null; end $$;
...
set status = coalesce(nullif(status, ''), 'active'),
    ...
    image_urls = case when image_urls is null or jsonb_typeof(image_urls) <> 'array' then '[]'::jsonb else image_urls end,
...
set routing_mode = coalesce(nullif(routing_mode, ''), 'seller_approve'),
...
create policy "service_role_all_suppliers" on public.suppliers for all using (public.is_service_role()) with check (public.is_service_role());
drop policy if exists "service_role_all_supplier_products" on public.supplier_products;
create policy "service_role_all_supplier_products" on public.supplier_products for all using (public.is_service_role()) with check (public.is_service_role());
drop policy if exists "service_role_all_supplier_orders" on public.supplier_orders;
create policy "service_role_all_supplier_orders" on public.supplier_orders for all using (public.is_service_role()) with check (public.is_service_role());
